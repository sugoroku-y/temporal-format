...
これはJavaScriptで日付や時刻を扱うための実装で、Temporalのインスタンスを文字列にシリアライズ/解析するための関数を提供するパッケージです。
...
不適切な書式文字列は型関数でチェックされるため、TypeScriptコンパイルエラーになります。

たとえば、`QQQ` のようなサポートされないトークンを含む文字列を渡すと、VS Code上で次のようにエラーが表示されます。

![VS Codeでの不適切な書式文字列エラー](images/invalid-format-string-error.png)

このチェックは実行時ではなく、型レベルで行われるため、開発中に早期に問題を発見できます。
